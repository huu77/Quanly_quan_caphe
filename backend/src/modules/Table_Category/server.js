...
const getCategoryServer = async (id) => {
...
const getMuiltiCategoryServer = async () => {
...
const createCategoryServer = async (name) => {
...
const UpdateCategoryServer = async ({ id, name }) => {
  if (!name || typeof name !== "string" || !name.trim()) {
    return ResponseStatus.createResponse(400, {
      message: "Invalid name provided.",
    });
  ...
  try {
    ...
    // Kiểm tra số lượng bản ghi bị ảnh hưởng
    if (result.affectedRows === 0) {
      return ResponseStatus.createResponse(404, { message: "Category not found." }); // Không tìm thấy bản ghi để cập nhật
    }
...
const deleteCategoryServer = async (id) => {
  ...
  try {
    ...
    if (results.length === 0) {
      ...
      return ResponseStatus.createResponse(404, { message: "Category not found." });
    }
