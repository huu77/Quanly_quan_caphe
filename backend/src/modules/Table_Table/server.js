...
const createTableServer = async (name) => {
  ...
  try {
    ...
    if (result.affectedRows === 0) {
      return ResponseStatus.createResponse(500, { message: 'Failed to create Table.' }); // Có lỗi khi thêm bản ghi
    }
...
const UpdateTableServer = async ({id,name}) => {
  if (!name || typeof name !== 'string' || !name.trim()) {
    return ResponseStatus.createResponse(400, { message: 'Invalid name provided.' });
  }
  ...
  try {
    ...
    // Kiểm tra số lượng bản ghi bị ảnh hưởng
    if (result.affectedRows === 0) {
      return ResponseStatus.createResponse(404, { message: 'Table not found.' }); // Không tìm thấy bản ghi để cập nhật
    }
...
const deleteTableServer = async(id) => {
  ...
  try {
    ...
    if (deleteResult.affectedRows === 0) {
      ...
      return ResponseStatus.createResponse(500, { message: 'Failed to delete Table.' });
    }

    // Trả về kết quả thành công với mã trạng thái 200 (OK)
    return ResponseStatus.createResponse(200, { message: 'Table deleted successfully.' });
