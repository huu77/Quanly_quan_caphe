...
const createProfileServer = async (account_id, firstname, lastname, phone, address, cccd) => {
    ...
    const sql = "INSERT INTO Profile (account_id, firstname, lastname,  phoneNumber, address, CCCD) VALUES (?, ?, ?, ?, ?, ?)";

    try {
        const [result] = await pool.query(sql, [account_id, firstname, lastname, phone, address, cccd]);
...
const updateProfileServer = async (account_id, firstname, lastname, phone, address, cccd) => {
    if (!account_id || typeof account_id !== "number") {
        return ResponseStatus.createResponse(400, {
            message: "Invalid account_id provided.",
        });
    ...
    const sql = "UPDATE Profile SET firstname = ?, lastname = ?, phoneNumber = ?, address = ?, CCCD = ? WHERE account_id = ?";

    try {
        const [result] = await pool.query(sql, [firstname, lastname, phone, address, cccd, account_id]);
