  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
      <div className="bg-white rounded-lg p-5 w-full max-w-md">
        <h2 className="text-xl font-bold mb-4 text-red-700">Order History</h2>

        <div className="max-h-96 overflow-y-auto">
          <ul>
            {groupedProducts.products.length > 0 ? (
              groupedProducts.products.map((product) => (
                <div key={product.id} className="relative flex border p-4 rounded-lg shadow mb-2">
                  <div className="flex-1 pr-4">
                    <h3 className="text-xl font-semibold text-gray-900">{product.name}</h3>
                    <p className="text-gray-700">{product.description}</p>
                    <p className="text-gray-900 font-bold">${product.price.toFixed(2)}</p>
                    <p className="text-gray-900 font-bold">Quantity: {product.quantity}</p>
                  </div>
